...
from connect4_ml import create_board, drop_piece, is_valid_location, get_next_open_row, check_winner, evaluate_board

class ConnectFourApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Connect Four - ML Approach")
    ...
    def drop_piece(self, col):
        ...
        if is_valid_location(self.board, col):
            ...
            drop_piece(self.board, col, self.current_player)
        ...
        for row in range(6):
            for col in range(7):
                if self.board[row][col] == 0:
                    self.cells[row][col].config(bg="white", text="")
                elif self.board[row][col] == 1:
                    self.cells[row][col].config(bg="blue", text="Human", fg="white")
                else:
                    self.cells[row][col].config(bg="red", text="AI", fg="white")
